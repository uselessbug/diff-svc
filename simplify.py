...
import torch
from infer_tools.infer_tool import mkdir
def get_model_folder(path):
    model_lists = os.listdir(path)
    res_list = []
    filter_list = ["hubert", "xiaoma_pe", "hifigan", "checkpoints", ".yaml", ".zip",".pt","clean"]
...
def simplify_pth(model_name, proj_name, output_path):
    ...
    torch.save({'state_dict': checkpoint_dict['state_dict']}, output_path)
